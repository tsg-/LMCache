...
def render_metrics(counters: Mapping[str, int], success: bool, timestamp: int) -> str:
    ...
    if success:
        ...
        for name in REQUIRED_COUNTERS:
            lines.append(
                'acc_telemetry_bytes_total{counter="%s",ulp="rdma"} %d'
                % (name, counters[name])
            )
    lines.extend(
        [
            ...
            f"acc_telemetry_collector_success {int(success)}",
            ...
            f"acc_telemetry_collector_timestamp_seconds {timestamp}",
        ]
    )
    return "\n".join(lines) + "\n"
...
def write_textfile(output_dir: Path, contents: str) -> None:
...
def main() -> int:
    ...
    try:
        ...
        write_textfile(
            output_dir,
            render_metrics({}, success=False, timestamp=timestamp),
        )
        return 1

    write_textfile(
        output_dir,
        render_metrics(counters, success=True, timestamp=timestamp),
    )
    return 0
