...
def test_phase_end_time_defaults_to_zero_until_the_window_closes() -> None:
    """A live query must be able to tell an open window from a closed one.

    ``phase_ended_at`` is 0.0 on a freshly constructed result -- run_sustained_window
    has not run yet -- so the scrape must render that as a real 0.0 sample,
    not omit the series, or a query gating on it would see no data instead
    of "still open".
    """
    state = BenchMetricsState()
    result = _result()
    result.phase_started_at = 1_700_000_000.0
    state.register(result.operation, result, PHASE_MEASURED)

    text = _scrape(state)

    labels = {"operation": "Store", "phase": PHASE_MEASURED}
    assert _series(text, "phase_start_time_seconds", **labels) == 1_700_000_000.0
    assert _series(text, "phase_end_time_seconds", **labels) == 0.0

    result.phase_ended_at = 1_700_000_030.0
    text = _scrape(state)
    assert _series(text, "phase_end_time_seconds", **labels) == 1_700_000_030.0
